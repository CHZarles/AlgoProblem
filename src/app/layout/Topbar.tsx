...
import { Check, ChevronsUpDown, FileText, Leaf, Moon, Rows3, Search, Sparkles, Sun, Zap } from "lucide-react";
...
export function Topbar({
  ...
  const meta = (() => {
    ...
    if (theme.preference === "neon") return { Icon: Zap, label: "Neon", iconClass: "text-cyan-300" };
        ...
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Portal>
            ...
            >
              ...
              <DropdownMenu.Item
                onSelect={() => theme.setPreference("neon")}
                className={cn(
                  "flex cursor-pointer items-center justify-between rounded-lg px-2.5 py-2 text-sm outline-none",
                  "text-slate-200 data-[highlighted]:bg-white/6 data-[highlighted]:text-slate-50",
                  theme.preference === "neon" && "bg-white/4",
                )}
              >
                <span className="inline-flex items-center gap-2">
                  <span className="grid h-7 w-7 place-items-center rounded-lg bg-cyan-500/12 shadow-[0_0_0_1px_rgba(34,211,238,0.22)]">
                    <Zap className="h-4 w-4 text-cyan-300" />
                  </span>
                  <span className="font-medium">Neon</span>
                </span>
                {theme.preference === "neon" ? <Check className="h-4 w-4 text-sky-500" /> : null}
              </DropdownMenu.Item>
              <DropdownMenu.Item
